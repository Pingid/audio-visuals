...
function draw(){
  ...
  const addSpectrum = (points, specs) => points.map(({ x, y }, i) => ({
    x: x + x * specs[i % specs.length],
    y: y + y * specs[i % specs.length]
  }))
  ...
  shape
    .reverse()
    .map((p, i) => {
      stroke(color(spec[i % spec.length] * 10 + 100, 200, 100))
      ellipse(p.x, p.y, spec[i % spec.length] * .5, spec[i % spec.length] * .5)
      ellipse(p.x, p.y, spec[i % spec.length] * 1, spec[i % spec.length] * 1)
      ellipse(p.x, p.y, spec[i % spec.length] * 1 + 3, spec[i % spec.length] * 1 + 3)
      ellipse(p.x, p.y, spec[i % spec.length] * 2 + 5, spec[i % spec.length] * 2 + 5)
    })
